...
function ShareEvent({ eventData, onClose, onCopy }) {

    const text = `
        ${eventData?.eventName}
        ${format(new Date(eventData?.selectedDate.seconds * 1000), "PPpp") }
    ...
    const shareOnSocialMedia = (platform) => {
        switch (platform) {
            ...
            case 'twitter':
                // Share on Twitter
                window.open(`https://twitter.com/intent/tweet?url=${encodeURIComponent(eventData?.locationUrl)}&text=${encodeURIComponent(`${text}`)}`, '_blank');
                break;
            case 'linkedin':
                // Share on LinkedIn
                window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(eventData?.locationUrl)}&text=${encodeURIComponent(`${text}`)}`, '_blank');
                break;
            case 'whatsapp':
                // Share on WhatsApp
                          ...
                          <div className="my-4">
                            ...
                            <div className="border-2 border-gray-200 flex justify-between items-center mt-4 py-2">
                                ...
                                >
                                  ...
                                  <input className="w-full outline-none bg-transparent ml-1" type="text" placeholder="link" defaultValue={eventData && eventData?.locationUrl}/>
                                  <button onClick={() => onCopy()} className="bg-indigo-500 text-white rounded text-sm py-2 px-5 mr-2 hover:bg-indigo-600">
                                        Copy
                                    </button>
                            </div>
                          </div>
